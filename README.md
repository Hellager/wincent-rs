...
- **Frequent Folders removal**: `remove_item(..., QuickAccess::FrequentFolders)` removes pinned folders by unpinning them. For unpinned frequent entries, it pins first and then removes through version-specific Explorer Shell verbs: Windows 10 removes with `unpinfromhome`, while Windows 11 uses `pintohome` as a toggle. If the second step fails, the folder may remain pinned.
