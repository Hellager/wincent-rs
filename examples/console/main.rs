...
use wincent::{
    predule::{QuickAccessManager, QuickAccess, WincentResult},
    error::WincentError,
};
...
fn show_welcome() {
    ...
    println!("{}{}║                                                  ║{}", BLUE, BOLD, RESET);
    println!("{}{}║   __      __.__                      __          ║{}", BLUE, BOLD, RESET);
    println!("{}{}║  /  \\    /  \\__| ____   ____   _____/  |_        ║{}", BLUE, BOLD, RESET);
    println!("{}{}║  \\   \\/\\/   /  |/    \\_/ ___\\_/ __ \\   __\\       ║{}", BLUE, BOLD, RESET);
    println!("{}{}║   \\        /|  |   |  \\  \\___\\  ___/|  |         ║{}", BLUE, BOLD, RESET);
    println!("{}{}║    \\__/\\  / |__|___|  /\\___  >\\___  >__|         ║{}", BLUE, BOLD, RESET);
    println!("{}{}║         \\/          \\/     \\/     \\/              ║{}", BLUE, BOLD, RESET);
    println!("{}{}║                                                  ║{}", BLUE, BOLD, RESET);
    println!("{}{}║           Windows Quick Access Manager           ║{}", BLUE, BOLD, RESET);
    println!("{}{}║                                                  ║{}", BLUE, BOLD, RESET);
    println!("{}{}╚══════════════════════════════════════════════════╝{}", BLUE, BOLD, RESET);
    println!();
}
...
async fn check_feasibility(manager: &QuickAccessManager) -> WincentResult<()> {
    ...
    if feasible {
        ...
        spinner.complete(false, "Some operations may be restricted, check system settings");
    }
...
async fn add_file_to_recent(manager: &QuickAccessManager) -> WincentResult<()> {
    ...
    match manager.add_item(&path, QuickAccess::RecentFiles).await {
        Ok(_) => {
...
async fn pin_folder_to_frequent(manager: &QuickAccessManager) -> WincentResult<()> {
    ...
    match manager.add_item(&path, QuickAccess::FrequentFolders).await {
        Ok(_) => {
...
async fn unpin_folder_from_frequent(manager: &QuickAccessManager) -> WincentResult<()> {
    ...
    match manager.remove_item(&path, QuickAccess::FrequentFolders).await {
        Ok(_) => {
...
async fn query_and_display_items(manager: &QuickAccessManager, qa_type: QuickAccess) -> WincentResult<()> {
    ...
    match manager.get_items(qa_type).await {
        Ok(items) => {
            ...
            println!("\n{}{}{} List ({} items):{}", YELLOW, BOLD, type_name, items.len(), RESET);
...
async fn empty_items(manager: &QuickAccessManager, qa_type: QuickAccess) -> WincentResult<()> {
    ...
    match manager.empty_items(qa_type).await {
        Ok(_) => {
