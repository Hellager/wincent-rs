...
use crate::{
    ...
    handle::{add_file_to_recent_with_api, validate_path, PathType},
...
impl QuickAccessManager {
    ...
    pub async fn new() -> WincentResult<Self> {
        ...
        let check_result = tokio::task::spawn_blocking(check_script_feasible).await??;

        if !check_result {
            let _ = fix_script_feasible();
            let recheck = tokio::task::spawn_blocking(check_script_feasible).await??;
    ...
    async fn check_query_feasible_async(&self) -> WincentResult<bool> {
        ...
        .await
        .map_err(|_| WincentError::Timeout("Query feasibility check timed out".to_string()))??;
    ...
    async fn check_handle_feasible_async(&self) -> WincentResult<bool> {
        ...
        .await
        .map_err(|_| WincentError::Timeout("Handle feasibility check timed out".to_string()))??;
    ...
    pub async fn add_item(&self, path: &str, qa_type: QuickAccess) -> WincentResult<()> {
        ...
        match qa_type {
            ...
            QuickAccess::FrequentFolders => {
                ...
                let result = self
                    .executor
                    .execute(PSScript::PinToFrequentFolder, Some(path.to_string()))
                    .await?;
                if !result.is_empty() {
                    return Err(WincentError::ScriptFailed(format!(
                        "Failed pinning folder: {}",
                        path
                    )));
                }
            }
    ...
    pub async fn remove_item(&self, path: &str, qa_type: QuickAccess) -> WincentResult<()> {
        ...
        match qa_type {
            QuickAccess::RecentFiles => {
                ...
                let result = self
                    .executor
                    .execute(PSScript::RemoveRecentFile, Some(path.to_string()))
                    .await?;
                if !result.is_empty() {
                    return Err(WincentError::ScriptFailed(format!(
            ...
            QuickAccess::FrequentFolders => {
                ...
                let result = self
                    .executor
                    .execute(PSScript::UnpinFromFrequentFolder, Some(path.to_string()))
                    .await?;
                if !result.is_empty() {
                    return Err(WincentError::ScriptFailed(format!(
                        "Failed unpinning folder: {}",
                        path
                    )));
                }
            }
    ...
    pub async fn empty_items(&self, qa_type: QuickAccess) -> WincentResult<()> {
        ...
        match qa_type {
            ...
            QuickAccess::FrequentFolders => {
                ...
                self.executor
                    .execute(PSScript::EmptyPinnedFolders, None)
                    .await?;
            }
            QuickAccess::All => {
...
mod tests {
    ...
    async fn test_item_management_cycle() -> WincentResult<()> {
        ...
        manager
            ...
            .await?;
        let exists = manager
            .check_item(file_path, QuickAccess::RecentFiles)
            .await?;
        ...
        let temp_dir = tempfile::Builder::new().prefix("wincent-test-").tempdir()?;
        let dir_path = temp_dir.path().to_str().unwrap();

        manager
            .add_item(dir_path, QuickAccess::FrequentFolders)
            .await?;
        let exists = manager
            .check_item(dir_path, QuickAccess::FrequentFolders)
            .await?;
        assert!(exists, "Directory should be added");

        manager
            .remove_item(dir_path, QuickAccess::FrequentFolders)
            .await?;
        let exists = manager
            .check_item(dir_path, QuickAccess::FrequentFolders)
            .await?;
    ...
    async fn test_collection_clearance() -> WincentResult<()> {
        ...
        let frequent_folders = manager.get_items(QuickAccess::FrequentFolders).await?;
        assert!(
            frequent_folders.is_empty(),
            "Frequent folders should be cleared"
        );
