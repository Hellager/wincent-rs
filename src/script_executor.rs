...
impl ScriptExecutor {
    ...
    pub async fn execute_ps_script_async(
        ...
        }).await.map_err(|e| WincentError::AsyncExecution(e.to_string()))??;
    ...
    pub async fn execute_with_timeout(
        ...
        match tokio::time::timeout(Duration::from_secs(timeout_secs), execution).await {
            Ok(result) => result,
            Err(_) => Err(WincentError::Timeout(format!("Script execution timed out after {} seconds", timeout_secs))),
        }
    }
}
...
impl QuickAccessDataFiles {
    ...
    fn new() -> WincentResult<Self> {
        ...
        let frequent_folders_path = automatic_dest_dir.join("f01b4d95cf55d32a.automaticDestinations-ms");
    ...
    fn get_recent_files_modified_time(&self) -> WincentResult<SystemTime> {
        if self.recent_files_path.exists() {
            let metadata = fs::metadata(&self.recent_files_path)
    ...
    fn get_frequent_folders_modified_time(&self) -> WincentResult<SystemTime> {
        if self.frequent_folders_path.exists() {
            let metadata = fs::metadata(&self.frequent_folders_path)
                .map_err(WincentError::Io)?;
            Ok(metadata.modified().unwrap_or(SystemTime::now()))
        } else {
            Ok(SystemTime::now())
...
impl CachedScriptExecutor {
    ...
    pub async fn execute_with_timeout(
        ...
        match tokio::time::timeout(Duration::from_secs(timeout_secs), self.execute(script_type, parameter)).await {
            Ok(result) => result,
            Err(_) => Err(WincentError::Timeout(format!("Script execution timed out after {} seconds", timeout_secs))),
        }
    }
...
mod tests {
    ...
    fn test_cache_eligibility_check() {
        ...
        assert!(CachedScriptExecutor::should_cache(PSScript::QueryQuickAccess));
        assert!(CachedScriptExecutor::should_cache(PSScript::QueryRecentFile));
        assert!(CachedScriptExecutor::should_cache(PSScript::QueryFrequentFolder));
        
        // Non-cacheable script types
        assert!(!CachedScriptExecutor::should_cache(PSScript::RefreshExplorer));
        assert!(!CachedScriptExecutor::should_cache(PSScript::RemoveRecentFile));
        assert!(!CachedScriptExecutor::should_cache(PSScript::PinToFrequentFolder));
        assert!(!CachedScriptExecutor::should_cache(PSScript::UnpinFromFrequentFolder));
        assert!(!CachedScriptExecutor::should_cache(PSScript::CheckQueryFeasible));
        assert!(!CachedScriptExecutor::should_cache(PSScript::CheckPinUnpinFeasible));
    }
    ...
    async fn test_quick_access_file_tracking() -> WincentResult<()> {
        ...
        assert!(data_files.recent_files_path.to_string_lossy().contains("5f7b5f1e01b83767"));
        assert!(data_files.frequent_folders_path.to_string_lossy().contains("f01b4d95cf55d32a"));
